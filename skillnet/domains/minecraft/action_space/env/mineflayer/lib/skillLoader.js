function inject(bot) {
    ...
    bot.useOn = async (entity) => {
        // Null check: prevent null pointer errors
        if (!entity || !entity.position) {
            const err = new Error("Cannot useOn: entity is null or has no position");
            err.code = "NULL_ENTITY_ERROR";
            bot.chat("Error: Invalid entity object (null or no position)");
            throw err;
        }
    ...
    bot.activateBlock = async (block) => {
        ...
        if (!block || !block.position) {
            const err = new Error("Cannot activateBlock: block is null or has no position");
            err.code = "NULL_BLOCK_ERROR";
            bot.chat("Error: Invalid block object (null or no position)");
            throw err;
        }
