...
async function exploreUntil(
    ...
    return new Promise((resolve, reject) => {
        ...
        const explore = () => {
            ...
            if (!bot.entity) {
                console.warn("[exploreUntil] bot.entity is undefined, skipping exploration iteration");
                return;  // skip this iteration, wait for respawn
            }

            // Check callback FIRST — if condition met, skip unnecessary goal change
            try {
            ...
            // Yield to active goto() navigation — prevents GoalChanged race condition
            // isMoving() returns true when pathfinder has an active computed path
        ...
        explore();  // Run first tick immediately (no 2s dead zone)
